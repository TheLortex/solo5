...
Options:
    ...
    --platform=host | PLATFORM
        Selects for which platform solo5 needs to be built.
        Supported platforms are 'host' and 'aarch64-linux-gnu'.

    --ocaml=static
        Selects which toolchain flags are used when building the bindings.
EOM
    exit 1
}
...
if [ "${CONFIG_PLATFORM}" = "host" ]; then
    CONFIG_CROSS_COMPILE=
    CC=${CC:-cc}
    LD=${LD:-ld}
    OBJCOPY=${OBJCOPY:-objcopy}
else
    CONFIG_CROSS_COMPILE=1
    # [Linux] Cross-toolchains normally come with a arch-vendor-os-gcc, so
    # default to that in this case, and figure out something for clang-based
    # systems later.
    CC=${CONFIG_PLATFORM}-${CC:-gcc}
    LD=${CONFIG_PLATFORM}-${LD:-ld}
    OBJCOPY=${CONFIG_PLATFORM}-${OBJCOPY:-objcopy}
fi
...
MAKECONF_OBJCOPY=${OBJCOPY}
MAKECONF_CC=${CC}
MAKECONF_HOST_CC=${HOST_CC}
MAKECONF_LD=${LD}
